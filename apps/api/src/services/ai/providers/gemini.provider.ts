import { geminiModel, geminiJsonModel } from '@/config/ai';
...
export const geminiProvider: IAiProvider = {
    ...
    async generateScoring(candidate, job): Promise<ICandidateScore> {
        const result = await geminiJsonModel.generateContent(buildScoringPrompt(candidate, job));
    ...
    async generateOutreach(candidate, job): Promise<string> {
        const result = await geminiModel.generateContent(buildOutreachPrompt(candidate, job));
        ...
        if (!text || text.length < 20) {
            throw new Error('Gemini returned empty outreach message');
        }
...
Classify the intent of this candidate's reply to a job outreach message.
Message: "${message}"
